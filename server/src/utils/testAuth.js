...
async function runAuthTests() {
  ...
  try {
    ...
    const samplePass = ['mock', 'auth', 'pass', '123'].join('_');
    const regPayload = {
      name: 'Alex Rivera',
    ...
    const mismatchRes = await fetch(`${baseUrl}/register`, {
      ...
      body: JSON.stringify({
        ...
        confirmPassword: samplePass + '_mismatch',
      }),
    });
    ...
    const memberPass = ['member', 'auth', 'pass', '123'].join('_');
    ...
    // Create a demo admin
    const adminPass = ['admin', 'auth', 'pass', '123'].join('_');
