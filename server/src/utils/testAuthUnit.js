...
async function runUnitTests() {
  ...
  const plainPassword = 'SuperSecretLeaderPassword#2026';
  const salt = 10;
  ...
  assert(await bcrypt.compare(plainPassword, hash), 'bcrypt.compare resolves true with matching password');
  assert(!(await bcrypt.compare('WrongPassword#2026', hash)), 'bcrypt.compare rejects incorrect password');
